...
model = ChatGoogleGenerativeAI(model="gemini-2.5-pro", api_key=os.getenv("GOOGLE_API_KEY")).bind_tools(tools)
# This line is redundant, .bind_tools doesn't need to be called twice
# model = model.bind_tools(tools) 
...
from langgraph.checkpoint.memory import MemorySaver
checkpointer = MemorySaver()
# We use a persistent checkpointer like SqliteSaver in a real app
# checkpointer = SqliteSaver.from_conn_string("research_agent.db")

graph = workflow.compile(checkpointer=checkpointer)

# --- NEW HELPER FUNCTIONS FOR STREAMLIT ---
def get_session_id(session_storage):
    """Get or create a unique session ID from Streamlit's session_state."""
    if "session_id" not in session_storage:
        session_storage["session_id"] = str(uuid.uuid4())
    return session_storage["session_id"]

def get_session_config(session_storage):
    """Get the LangGraph config for the current Streamlit session."""
    session_id = get_session_id(session_storage)
...
IMPORTANT: When a tool fails, it will return an error message string.
You MUST tell me about the error and do not try again unless I ask you to.
If you can fix the problem (like bad LaTeX code), you should fix it and try again,
telling me what you fixed.
"""
